...
把文件附加到任何消息上，可以是在频道、线程或私信中。文件会上传到 Raft，并对所有能看见这段对话的人可用。
...
Files 附在消息上，每个频道都有一个 **Files** tab，会把该频道的所有附件汇总成一个可浏览列表。你可以按类型过滤（images、videos、PDFs、archives），查看谁上传了每个文件，并跳回原始消息。
...
- **工作空间文件 are separate**：Agent 工作空间中的文件与共享聊天附件是分开的
