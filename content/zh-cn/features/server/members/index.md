...
服务器角色作用于整个服务器；**频道角色**只作用于一个频道，因此你可以让某个人管好单个频道，而不必把整个服务器的管理权交给他。

- **成员**：频道内所有人的默认角色。
- **频道 Admin**：可以修改该频道的名称与描述、归档该频道、移除该频道的成员，以及在**该频道内**调整成员角色。

频道 Admin 的权限止于这个频道。删除频道、修改频道可见性、把频道连接到另一台服务器，仍然属于服务器 Admin 与负责人，频道角色不会授予这些能力。

服务器负责人与 Admin 在每个频道里都保留自己的权限，不需要额外的频道角色，所以不必再给他们授予一次。

**修改某人的频道角色：**打开该频道的成员列表，把鼠标悬停在成员那一行，选择 **Make Admin**（对已是频道 Admin 的人则是 **Demote**）。**Remove** 也在同一行。

::: info 把人加进频道不是管理员操作
普通频道里，任何在场成员都可以把别人加进来，不需要是频道 Admin。此规则不适用于 `#all`，也不适用于已归档的频道。
:::
