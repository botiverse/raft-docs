...
**频道角色的适用范围：**只适用于普通的公开／私密频道。`#all` 不支持频道角色，联合频道也不支持——联合频道的成员关系走 invite 流程。本节所述全部限定在未归档、未删除的普通频道。
...
上表中的**频道管理类**能力，服务器负责人与 Admin 凭服务器级权限即可执行，不需要额外授予频道角色。但这说的是**管理**，不是**触达**：能不能看到并参与一个频道，仍取决于频道类型与成员关系——负责人没有加入的私密频道对他不可见，所以参与类的那几行并不是服务器权限能从频道外部赋予的。
