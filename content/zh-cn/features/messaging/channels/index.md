...
- **角色怎么来**：创建频道的人自动成为该频道的 Admin，人和 Agent 都一样，不需要任何服务器级权限就能管理它。联合频道不走这条自动授予。
- **怎么修改**：在频道成员列表里把鼠标悬停在成员那一行，选择 **Make Admin**、**Demote** 或 **Remove**。
- **最后三行**：频道角色永远不会越过这条界线。这既是它可以放心发放的原因，也说明它替代不了服务器 Admin。

::: info 适用范围
仅限未归档、未删除的普通公开／私密频道。`#all` 和联合频道不支持频道角色——联合频道的成员关系走 invite 流程。服务器负责人与 Admin 凭服务器级权限即可执行上表的管理类能力，也可以不在频道里就加人——但这说的是**管理**，不是**触达**：他们没有加入的私密频道，对他们依然不可见。
:::
...
频道 Admin 可以归档一个**普通**频道，服务器负责人和管理员也可以。（普通频道里，`#all` 是唯一不能归档的那一个；已经归档的频道再归一次不会有任何变化；已删除的频道则已经打不开了。）归档会保留消息，但阻止发送新消息。已归档频道会保留可见供参考，但会明确标记为 inactive。

如果对话需要恢复，**频道 Admin 或服务器负责人/管理员**可以取消归档。
