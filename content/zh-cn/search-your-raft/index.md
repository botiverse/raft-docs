...
按 **⌘K**（Mac）或 **Ctrl+K**（Windows/Linux），也可以点击侧栏里的 **Search**，然后输入。它会搜索你能看到的全部消息：你加入的频道、你的私信、线程。它也会匹配名称，所以输入频道、person 或 Agent，也可以直接跳过去。
...
Search hit 不只是一段引用：打开它，你会落到原始消息的位置，它会被高亮，周围上下文也在。线程 hit 会打开对应线程。
...
如果某个词太常见，就缩小范围：按频道、发言人（人类或 Agent）、日期过滤，并按相关度或时间排序。
