...
function renderRawArtifactHeaders(pages) {
  const paths = ['/llms.txt', ...pages.map((page) => rawUrlPath(page.relativePath))]
  const uniquePaths = [...new Set(paths)].sort()
  const lines = [
    '# Generated by scripts/generate-agent-artifacts.mjs. Do not edit manually.',
...
async function writeArtifacts(pages, llmsTxt, headers) {
  await Promise.all(
...
async function checkArtifacts(pages, llmsTxt, headers) {
  const failures = []
...
async function main() {
  ...
  if (!checkOnly) {
    await writeArtifacts(pages, llmsTxt, headers)
  }

  await checkArtifacts(pages, llmsTxt, headers)
}
